...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
        #h = (Xi.loc[i])
       # h_std = np.std(h)
        calc = calcSumValue(X,(Xi.loc[i]))
        calc = wt * calc
        exponentValue = np.exp(calc)
       # exponentValue=0.0
        numerator +=(Xi.iloc[i][-1] * exponentValue)
        denominator += exponentValue
    #print(count)
    prob = numerator/denominator
    return prob

def calcSumValue(g,h):
  g = g[:-1]
  h = h[:-1]
  calc = 0.0
  for i in range(0,len(g)):
    calc += ((g[i]-np.mean(g))*(h[i]-np.mean(h)))

  return  calc / (len(g) * np.std(g) * np.std(h))
